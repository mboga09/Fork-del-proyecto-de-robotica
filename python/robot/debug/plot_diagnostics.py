...
def generate_transfer_debug_plots(
    *,
    ...
    if not wells:
        raise ValueError("No wells selected for plotting.")
    ...
    _emit(status_callback, f"Debug plot trajectory: {path_plot_path}")
...
def _plot_robot_initial_final(robot, q_initial, q_final, last_well: str, output_path: Path) -> None:
    ...
    ax.plot(initial_points[:, 0], initial_points[:, 1], initial_points[:, 2], marker="o", label="q initial")
    ax.plot(final_points[:, 0], final_points[:, 1], final_points[:, 2], marker="o", label=f"final pose {last_well}")

    ax.scatter(initial_points[-1, 0], initial_points[-1, 1], initial_points[-1, 2], label="initial TCP")
    ax.scatter(final_points[-1, 0], final_points[-1, 1], final_points[-1, 2], label="final TCP")

    ax.set_title("PRR model: initial vs final")
    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.set_zlabel("Z [m]")
    ax.legend()
...
def _plot_path_arrows_3d(points: np.ndarray, output_path: Path) -> None:
    if points.shape[0] < 2:
        raise ValueError("At least two points are required for plotting.")
    ...
    ax.scatter(points[0, 0], points[0, 1], points[0, 2], s=60, label="start")
    ax.scatter(points[-1, 0], points[-1, 1], points[-1, 2], s=60, label="end")

    ax.set_title("3D path generated by PathPlanner")
    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.set_zlabel("Z [m]")
    ax.legend()
...
def _robot_link_points(robot, q) -> np.ndarray:
    ...
    p2 = p1 + np.array([l2 * np.cos(theta2), l2 * np.sin(theta2), 0.0])
    p3 = p2 + np.array([l3 * np.cos(theta2 + theta3), l3 * np.sin(theta2 + theta3), tool_z])
